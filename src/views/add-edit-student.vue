...
export default {
  ...
  methods: {
    submit() {
      ...
      this.$v.studentAddress.$touch();
      this.$v.studentPhone.$touch();
      ...
      axios.post(method, studentData).then((res) => {
        ...
        this.error = null;
        if (res.data.done) {
          this.$store.dispatch("writeMessage", "تم اضافة الطالب بنجاح");
          if (res.data.student) {
            console.log(res.data.student._id.split("").slice(9, 24).join(""));
            this.barcodeId = res.data.student._id.split("").slice(9, 24).join("");
          }
        } else {
          const error = res.data.error.includes("E11000")
            ? "هذا الطالب موجود بالفعل"
            : res.data.error;

          this.$store.dispatch("writeMessage", error);
        }
      });
    },
    toggleGroup(id) {
